...
throws_ok { $v->validate_csv('t/data/03_if.csv') }
  qr/fields with an 'if' dependency/, 'exception when validating against bad config ("if" field is not boolean)';

# "if" relationships
$v->config_name('if');
is( $v->validate_csv('t/data/03_if.csv'), 0, 'broken "if" relationship input file marked as invalid' );
...
is( $v->validate_csv('t/data/03_one_of.csv'), 0, 'broken "one of" relationship input file marked as invalid' );
...
is( $v->validate_csv('t/data/03_some_of.csv'), 0, 'broken "some of" relationship input file marked as invalid' );
