...
( $rv, $stdout, $stderr ) = run_script( $script, [ qw(-c t/data/07_manifest.conf) ] );

like( $stderr, qr/ERROR: you must specify an input file/, 'got expected error message with just -c flag' );

run_output_matches(
  $script,
  [ qw(-c t/data/07_manifest.conf t/data/07_broken_manifest.csv) ],
  [ qr/input data are .*?invalid/ ],
  [],
  'got expected "invalid" message with valid config and invalid manifest',
);

run_ok( $script, [ qw(-c t/data/07_manifest.conf t/data/07_working_manifest.csv) ],
  'runs ok with valid arguments' );

( $rv, $stdout, $stderr ) = run_script( $script, [ '-c', 't/data/07_manifest.conf', 't/data/07_working_manifest.csv' ] );
...
( $rv, $stdout, $stderr ) = run_script( $script, [ '-c', 't/data/07_manifest.conf', '-o', $filename, 't/data/07_working_manifest.csv' ] );
...
( $rv, $stdout, $stderr ) = run_script( $script, [ '-c', 't/data/07_manifest.conf', '-o', $filename, '-i', 't/data/07_working_manifest.csv' ] );
