...
use_ok('Bio::Metadata::Finder::Path');

# make sure we can't instantiate the object with unknown arguments
throws_ok { Bio::Metadata::Finder::Path->new(unknown => 'attr') }
...
is $p->db_root, 't/data/11_finder_path/root_dir', 'got expected path to root dir';

# missing db_subdirs mapping
my $returned_subdirs;
$p = Bio::Metadata::Finder::Path->new(config_file => 't/data/11_finder_path/no_subdirs.conf');
...
$p = Bio::Metadata::Finder::Path->new(config_file => 't/data/11_finder_path/no_template.conf');
...
$p = Bio::Metadata::Finder::Path->new(config_file => 't/data/11_finder_path/test.conf');
...
is $p->get_hierarchy_root_dir('pathogen_test_pathfind'),
  't/data/11_finder_path/root_dir/pathogen_test_pathfind/seq-pipelines',
  'got correct root path for test DB';
is $p->get_hierarchy_root_dir('pathogen_prok_track'),
  't/data/11_finder_path/root_dir/prokaryotes/seq-pipelines',
