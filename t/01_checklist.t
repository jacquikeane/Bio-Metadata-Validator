...
throws_ok { Bio::Metadata::Checklist->new( config_file => $nef, config_name => 'dummy' ) }
  qr/could not find the specified configuration file/, 'exception on missing config file';

throws_ok { Bio::Metadata::Checklist->new( config_file => 't/data/01_broken.conf', config_name => 'broken' ) }
...
my $c;
lives_ok { $c = Bio::Metadata::Checklist->new( config_file => 't/data/01_single.conf', config_name => 'one' ) }
...
lives_ok { $c = Bio::Metadata::Checklist->new( config_file => 't/data/01_single.conf' ) }
...
lives_ok { $c = Bio::Metadata::Checklist->new( config_file => 't/data/01_multiple.conf', config_name => 'one' ) }
...
$c = Bio::Metadata::Checklist->new( config_file => 't/data/01_single.conf' );
