...
use_ok('Bio::Metadata::Finder::Database');

# make sure we can't instantiate the object with unknown arguments
throws_ok { Bio::Metadata::Finder::Database->new(unknown => 'attr') }
...
my $d;
lives_ok { $d = Bio::Metadata::Finder::Database->new(environment => 'test', config_file => 't/data/12_finder_database/no_dbs.conf') }
...
my $expected_connection = {
  dbname => 't/data/12_finder_database/test.db',
  host   => 'test_db_host',
  port   => 3306,
  user   => 'ro_user',
  pass   => 'password',
...
$d = Bio::Metadata::Finder::Database->new(environment => 'prod', config_file => 't/data/12_finder_database/prod.conf');
